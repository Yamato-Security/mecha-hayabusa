...
- Added evtx reachability measurement (`state.py reach --eid-metrics`), so an investigation can state how much of the collected evtx the timeline could ever surface instead of assuming it is complete. A Hayabusa CSV holds only events that matched a rule, so everything no rule matched is missing from it while still present in the evtx. `hayabusa eid-metrics` output taken over the original corpus is compared against the timeline's `(channel, event id)` pairs and the absent ones are recorded, largest first. On a 7,928-file / 36.7 GiB corpus that is 1,959 of 2,072 pairs — 43,981,154 of 59,884,494 events (73.4%). Channel names are compared case-insensitively, because Hayabusa's own subcommands disagree on capitalisation; the import is refused outright if a timeline pair is missing from the metrics file, rather than recording a false figure. (#45) (@YamatoSecurity)
- Added the measured figure to the report appendix, together with the caveat that a timeline filtered by time, host, level or rule selection understates coverage, and `reach --none --reason ...` to record explicitly that the original evtx was unavailable. Both are reported rather than left to inference: "absent from the supplied timeline" is what the comparison establishes, and "no rule ever matched" is not. (#45) (@YamatoSecurity)
