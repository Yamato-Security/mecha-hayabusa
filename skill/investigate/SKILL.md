...
- **Exception — count-based correlation rules.** A few Hayabusa rules (`Failed Logins with Different Accounts from Single Source System`, `Rare Service Installations`, ...) emit one *aggregated* row per correlation window with an empty `RecordID`, so no single event can be cited. For those, and only those, set `"refs_unavailable": true` on the triage entry instead of `refs`. Gate G7 verifies the claim against the dataset: if any row of that rule does carry a RecordID, the verdict is rejected as unevidenced. Never use this to skip evidence for a rule you simply did not check
