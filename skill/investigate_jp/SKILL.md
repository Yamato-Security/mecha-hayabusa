...
- **比較可能なすべての識別子投影が一致していること。** 通常イベント行の組み合わせがメトリクスに無い場合、または相関行の具体的なイベントIDに同じ行の候補グラフ内で整合するコーパス辺が無い場合、値を黙って破棄せず取り込みを拒否する。表示チャネルにも同様に辺を要求する。ただし、その行が EventID の欠損（`-`/空）を明示している場合、その元レコードは定義上 `eid-metrics` に含まれないため例外とする。
