...
- ステートディレクトリ `STATE_DIR` は Step 1 で作成するレポート出力ディレクトリと同一。全ステートファイル（`manifest.json`, `rule_triage.json`, `clusters.json`, `findings.json`, `iocs.json`, `hosts.json`, `environment.json`, `reachability.json`, `queries.jsonl`, `verification_votes.jsonl`）はチャート・レポートと同じ場所に置かれる
...
### Step 5.6: Evtx 表現カバレッジの計測 ★提供されたタイムラインに表現されない識別子を把握する

Hayabusa の CSV は **検知**の成果物であり、元の evtx を完全に複製したものではない。CSV には、派生した相関要約を含むルール出力が記録され、元イベントがすべて独立した行として現れるわけではない。したがって「この識別子は提供されたタイムラインに表現されていない」と「このイベントはログに存在しない」は別の主張である。この Step が計測するのは前者だけである。
...
コーパスのイベントのうち、**提供されたタイムラインの識別フィールドに一意に表現されない** `(チャネル, イベントID)` に属する割合が記録される。結論を書く前に、確実に不在とされた組み合わせの上位項目を必ず確認すること。この数値はレポート付録にも出力され、読み手がタイムラインを evtx の完全な複製と誤解することを防ぐ。

**この主張は狭く読むこと。** 次のどちらかを満たす場合、組み合わせは一意に表現されている:

- 通常のイベント行が RecordID を持ち、チャネルとイベントIDをそれぞれ1つだけ示す場合
- 相関要約の候補グラフを同一コーパスのメトリクスと照合し、組み合わせが強制される場合。1チャネルまたは1イベントIDの要約では表示された各組み合わせが強制され、複数×複数でも表示ノードの候補が1本だけならその辺が強制される

RecordID を持つ直接イベント行で EventID が空欄なら `eid-metrics` が用いる具体的な `null` 識別子へ正規化し、リテラル `-` は通常の EventID として扱う。

相関要約に複数のチャネル**かつ**複数のイベントIDが含まれる場合、両者は独立して重複排除されたリストであり、対応関係は直接には分からない。インポーターは行ごとの候補グラフとコーパスを照合し、候補が1本のチャネルまたはイベントIDが強制する辺を表現済みとし、残る任意辺だけを**未確定**とする。未確定イベントを含めない下限と、すべて含める上限で表現ギャップを報告する。また、**時系列相関**では最初に参照されたルールの結果しか描画されず、別の必須識別子が表示フィールドにまったく現れないこともある。したがって、このメトリクスが示すのは CSV 内での識別子の表現状況であり、ルールセットの検知能力や、元イベントがルールに一致したかどうかではない。

ツールが隠さず明示する注意点が2つある:

- **両者は同じ evtx から、絞り込みなしで生成すること。** `state.py` は、絞り込みのないタイムラインと、時間・ホスト・レベル・ルールで限定されたタイムラインを区別できない。絞り込まれた実行では、同等の絞り込みなしのタイムラインなら表現される識別子も正当に除外される。その場合、報告値は提供されたファイル自体の性質としてのみ解釈すること。絞り込みなしのタイムラインにおける表現ギャップを過大評価する可能性がある。
- **比較可能なすべての識別子投影が一致していること。** 通常イベント行の組み合わせがメトリクスに無い場合、または相関行の具体的なイベントIDに同じ行の候補グラフ内で整合するコーパス辺が無い場合、値を黙って破棄せず取り込みを拒否する。表示チャネルにも同様に辺を要求するが、集約行に EventID の空コンポーネントがある場合だけ、そのコンポーネントに対応するチャネル要件を緩和する。この省略投影によって同じチャネル上の無関係な具体的IDが候補になることはない。RecordID を持つ直接イベント行では、空欄を `null` として照合し、リテラル `-` はそのまま通常値として扱う。
...
**表現はここから決まる。** evtx 自体を検索していない段階では、「提供されたタイムラインに一意に表現されていない」（確実な不在集合なら「提供されたタイムラインの識別フィールドに存在しない」）と記述すること。このメトリクスを「どのルールにも一致しなかった」「決して可視化できない」「ログに証拠がない」と読み替えてはならない。いずれも本計測では立証されない。ログレベルの不在を機械的に証明するには、本スキルがまだ取り込んでいない生 evtx の検索証跡が必要である。
