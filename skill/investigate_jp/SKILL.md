...
### Step 5.6: Evtx 到達性の計測 ★タイムラインが決して示せない範囲を把握する
...
コーパスのイベントのうち、**タイムラインのイベント識別フィールドに現れない** `(チャネル, イベントID)` に属する割合が記録される。結論を書く前に上位の項目を必ず確認すること。この数値はレポート付録にも出力され、読み手がタイムラインを完全なものと誤解することを防ぐ。

**この主張は狭く読むこと。** これは RecordID を持つ行に実際に現れた `(チャネル, イベントID)` についての記述であり、ルールセットが「検知し得る」範囲を示すものではない。相関行は RecordID を持たず要約にすぎない。Channel と EventID は互いに独立して構築された重複排除済みリストであり、さらに**時系列相関**では最初に参照されたルールの結果のみが描画されるため、`Sys/7045` が同様に必要であってもどこにも現れず `Sec/4624` だけが記載されることがある。したがって相関行はペアを保証せず、そこに現れるペアは**未確定**として報告される。真の不在は、提示された値以上、これに未確定分を加えた値以下である。

ツールが隠さず明示する注意点が2つある:

- **両者は同じ evtx から、絞り込みなしで生成すること。** `state.py` は、絞り込みのないタイムラインと、時間・ホスト・レベル・ルールで限定されたタイムラインを区別できない。絞り込まれたタイムラインは全コーパスのメトリクスの部分集合になるのが当然であり、除外された組み合わせも「存在しない」として計上される。絞り込みを行った場合、この数値は上限値として読むこと。
- **チャネル表記が一致していること。** タイムラインの組み合わせがメトリクス側に存在しない場合、誤った数値を記録せずに取り込みを拒否する。
...
**表現はここから決まる。** evtx 自体を検索していない段階で CSV が裏付けるのは「どのルールにも一致しなかった」「タイムラインに存在しない」であって、「ログに証拠がない」ではない。この区別を機械的に強制するには、本スキルがまだ取り込んでいない生 evtx の検索証跡が必要であり、それまでは分析者の責任である。
