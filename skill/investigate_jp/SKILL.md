...
**不在を主張する対象は宣言する。** トリアージの rationale や finding の summary で「存在しなかった」と述べる場合、その対象をデータとして当該エントリに明記する:

```json
{
  "rule_title": "...", "verdict": "false_positive",
  "rationale": "生の evtx でも evil.example.com への通信は確認されなかった。",
  "absence": ["evil.example.com", "10.0.0.1"]
}
```

**G12 は宣言された各対象について、ヒット0件の再検索を要求する。** 再検索が無ければ FAIL。再検索が**ヒットした**場合も FAIL であり、それは主張を裏付けるのではなく否定する。宣言することで初めて義務が検証可能になる。散文だけでは検証できないため、ゲートは宣言を基準とする。

evtx コーパスが利用できない場合、`reach --none --reason ...` は「実行できなかった検索」を免除する。ただし**既に記録された矛盾は取り消さない** — ヒットが記録されていれば、コーパスがマウントされているか否かに関わらず主張は否定される。

**宣言のない散文が不在の主張に見える場合は警告のみで、FAIL にはしない。** 語句マッチでは「ログに X の痕跡はない」と「どちらとも言えない」を確実に区別できず、その文がどの対象について述べているかも判定できない。したがって助言に留め、判断は分析者に委ねる。警告が妥当なら `absence` を宣言し、不適切なら無視してよい。

タイムラインに限定した主張（「タイムラインに痕跡はない」「どのルールにも一致しなかった」）は宣言不要である。これは CSV だけから言える弱い（そして正確な）主張であり、evtx まで遡っていない段階ではこの表現を使うこと。
